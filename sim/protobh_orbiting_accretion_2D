//========================================================================================
// 2D accretion-disk formation around orbiting stellar holes inside gaseous star clusters
...
//========================================================================================
//! \file protobh_orbiting_accretion_2D.cpp
//! \brief Orbiting BH accretion in 2D - simple harmonic elliptical orbit.
...
//!   v_r   =  - V_BH_R * cos(phi) - V_BH_psi * sin(phi)
//!   v_phi = -omega_BH * r + V_BH_R * sin(phi) - V_BH_psi * cos(phi)
//!
//! Source accelerations in BH frame:
//!   a_r   = - G * m_BH/r^2 - Omega^2 * r + omega_BH^2 * r + 2 * omega_BH * v_phi
//!   a_phi = - 2 * omega_BH * v_r - dotomega_BH * r
//========================================================================================
...
void ComputeGasVelocity(Real r, Real phi, const BHOrbitState &orb, Real &vr, Real &vphi);

Real ComputeXi0(Real psi0);

std::string GetSimulationDirectory(Real M_BH_Msun, Real B_pc, Real ecc_val, Real psi0_rad, int nx1, int nx2, 
                                   Real rho_cluster, Real nu_iso_code, Real R_inner_code, Real R_outer_code, Real t_end, 
                                   const std::string &base_path = "./data/2D");

std::string SetupOutputDirectory(ParameterInput *pin,
                                Real M_BH_Msun, Real B_pc, Real ecc_val, Real psi0_rad, int nx1, int nx2, 
                                Real rho_cluster, Real nu_iso_code, Real R_inner_code, Real R_outer_code, Real t_end,
...
void WriteSimulationParameters(const std::string& dir, Real M_BH_Msun, Real B_pc, Real ecc_val, Real psi0_rad, Real R_inner_code, 
                                Real R_outer_code, Real R_circ_code, Real Dt_inflow_yr, int nx1, int nx2, 
...
void Mesh::InitUserMeshData(ParameterInput *pin) {
    ...
    const Real R_circ_code = Omega_cgs * Omega_cgs * R_Bondi_max_cgs * R_Bondi_max_cgs * R_Bondi_max_cgs * R_Bondi_max_cgs
             / (G_cgs * M_BH_Msun * M_sun_cgs) / length_unit_cgs;

    // Initial phase from inertial angle psi0 
    xi0 = ComputeXi0(psi0_rad);

    // ---------- Setup output ----------
    std::string out_dir = SetupOutputDirectory(pin, M_BH_Msun, B_pc, ecc_in, psi0_rad, nx1, nx2, rho_cluster,
    ...
    WriteSimulationParameters(out_dir, M_BH_Msun, B_pc, ecc_in, psi0_rad, R_inner_code, R_outer_code, 
                              R_circ_code, Dt_inflow_yr, nx1, nx2, rho_cluster_cgs, rho_gas_cgs, epsilon, T_gas_K, mu, 
...
void MeshBlock::ProblemGenerator(ParameterInput *pin) {
    ...
    for (int k = ks; k <= ke; ++k) {
        for (int j = js; j <= je; ++j) {
            const Real phi = pcoord->x2v(j);
            for (int i = is; i <= ie; ++i) {
                const Real r = pcoord->x1v(i);

                Real vr, vphi;
                ComputeGasVelocity(r, phi, orb, vr, vphi);
                ...
                const Real KE = 0.5 * rho * (vr*vr + vphi*vphi);
...
void OuterX1_User(MeshBlock *pmb, Coordinates *pco, AthenaArray<Real> &prim,
    ...
    for (int k = kl; k <= ku; ++k) {
        for (int j = jl; j <= ju; ++j) {
            const Real phi = pco->x2v(j);
            for (int i = 1; i <= ngh; ++i) {
                const int  ig  = iu + i;
                const Real r_g = pco->x1v(ig);

                Real vr, vphi;
                ComputeGasVelocity(r_g, phi, orb, vr, vphi);

                prim(IDN, k, j, ig) = 1.0;
                prim(IPR, k, j, ig) = 1.0 / gamma_gas;
                prim(IVX, k, j, ig) = vr;
                prim(IVY, k, j, ig) = vphi;
                prim(IVZ, k, j, ig) = 0.0;
            }
        }
    }
...
//   a_r   = -Omega^2 r + omega^2 r + 2 omega v_phi - GM/r^2
//   a_phi = -2 omega v_r - omegadot r
//========================================================================================
void SourceFunction(MeshBlock *pmb, const Real time, const Real dt,
                    const AthenaArray<Real> &prim, const AthenaArray<Real> &prim_scalar,
                    const AthenaArray<Real> &bcc, AthenaArray<Real> &cons,
    ...
    for (int k = pmb->ks; k <= pmb->ke; ++k) {
        for (int j = pmb->js; j <= pmb->je; ++j) {
            for (int i = pmb->is; i <= pmb->ie; ++i) {
                ...
                const Real vph = prim(IVY, k, j, i);

                const Real a_r_G = - gm_code / (r * r);
                const Real a_r_I   = (-Omega2 + omega2) * r + 2.0 * orb.omega * vph;
                const Real a_phi_I = - 2.0 * orb.omega * vr - orb.omegadot * r;
                ...
                cons(IM2, k, j, i) += dt * rho * a_phi;
                cons(IEN, k, j, i) += dt * rho * (vr * a_r + vph * a_phi);
            }
        }
    }
}
...
//========================================================================================
// Helper: gas velocity in BH frame at (r, phi)  
//========================================================================================
void ComputeGasVelocity(Real r, Real phi, const BHOrbitState &orb,
                        Real &vr, Real &vphi) {
    const Real sp = std::sin(phi);
    const Real cp = std::cos(phi);

    vr   =  - orb.V_R  * cp - orb.V_psi * sp;

    vphi = -orb.omega * r + orb.V_R  * sp - orb.V_psi * cp;
}
...
std::string GetSimulationDirectory(Real M_BH_Msun, Real B_pc, Real ecc_val, Real psi0_rad,  
                                    int nx1, int nx2, Real rho_cluster, Real nu_iso_code,
                                    Real R_inner_code, Real R_outer_code, Real t_end,
                                    const std::string &base_path) {
    char subdir[256];
    std::snprintf(subdir, sizeof(subdir),
                  "mBH%.1f_B%.3f_ecc%.2f_psi0%.2f_nR%d_nP%d_rho%.1e/nu%.0e_NonI%d_Rinn%.1e_Rout%.2f_tend%.2f",
                  M_BH_Msun, B_pc, ecc_val, psi0_rad, nx1, nx2, rho_cluster,
...
std::string SetupOutputDirectory(ParameterInput *pin, Real M_BH_Msun, Real B_pc, Real ecc_val, Real psi0_rad, 
                                    int nx1, int nx2, Real rho_cluster, Real nu_iso_code, Real R_inner_code,
                                    Real R_outer_code, Real t_end,
                                    const std::string &out_dir_override) {
    ...
    const std::string dir = dir_expanded.empty()
        ? GetSimulationDirectory(M_BH_Msun, B_pc, ecc_val, psi0_rad,
                                 nx1, nx2, rho_cluster, nu_iso_code, R_inner_code, R_outer_code, t_end)
        : GetSimulationDirectory(M_BH_Msun, B_pc, ecc_val, psi0_rad,
                                 nx1, nx2, rho_cluster, nu_iso_code, R_inner_code, R_outer_code, t_end,
                                 dir_expanded);
    if (Globals::my_rank == 0) {
...
void WriteSimulationParameters(const std::string& dir, Real M_BH_Msun, Real B_pc, Real ecc_val, 
                                ...
                                int nx1, int nx2, Real rho_cluster_cgs, Real rho_gas_cgs, Real epsilon, 
